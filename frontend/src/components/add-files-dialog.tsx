    ...
    if (redactionType === "AI") {
      try {
        ...
        const res = await Promise.all(
          files.map(async (file) => await aiRedactFile(file)),
        );
        const zip_ = new zip();
        ...
        res.forEach((file, index) => {
          if (file) {
            ...
            folder?.file(
              name.substring(0, dot) + "-redacted" + name.substring(dot),
              file,
            );
          }
        });
    ...
    <DialogContent className={cn("max-w-screen-lg")}>
      ...
      <div className="flex gap-4 items-center h-[140px]">
        {files.map((file, index) => (
          <div
            key={index}
            className="bg-secondary w-fit p-4 rounded-md cursor-pointer relative"
          >
            ...
            <p className="text-xs mt-2 font-[500]">
              {(file.name.length >= 5
      ...
      <div>
        <label
          htmlFor="fileInput"
          className="flex flex-col gap-4 items-center justify-center bg-gray-200 rounded-md p-4 h-[300px] relative cursor-pointer"
        >
          <p className="font-[500] text-xl mb-4">
